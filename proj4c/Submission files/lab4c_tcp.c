...
void temp_reading(mraa_aio_context temperature, struct pollfd fds[]){
  ...
  while(run_flag){
    ...
    write_output(1, time_string, snum);
    ...
    if(fds[0].revents & POLLIN){
      ...
      if(log_file){
	write(1, buffer, read_size);//Write to the log file
      }
...
void read_input(char** buffer, int* read_size){
  *buffer = malloc(2000*sizeof(char));
  int buffer_size = 256;
  int read_status = 1;
  read_status = read(sockfd, *buffer+*read_size, buffer_size);    
  if(read_status < 0){
